...
export class ScalabilityModule {
    id = 'scalability';
    name = 'Scalability';
    async run(_target, config, engine, onFinding) {
        const findings = [];
        ...
        if (rateLimited === 0) {
            track(finding('NO_RATE_LIMIT', 'GET', '/', 'No rate limiting detected on burst', 'medium', `Burst of ${burstSize} requests: no 429 responses`, 'CWE-770', 5.3));
        }
        else {
            track(finding('RATE_LIMIT_DETECTED', 'GET', '/', 'Rate limiting present', 'info', `${rateLimited}/${burstSize} requests returned 429`, undefined, 1.0));
        }
        ...
        if (errors > 0) {
            track(finding('SOAK_TEST_FAILURES', 'GET', '/', 'Errors during sustained load', 'high', `${errors}/${soakRequests} requests failed (5xx or timeout)`, 'CWE-400', 6.5));
        }
        // 3. Throttle state observation
        const state = engine.getThrottleState();
        if (state === 'throttle') {
            track(finding('LATENCY_THROTTLE_TRIGGERED', 'GET', '/', 'Engine throttled due to latency', 'medium', 'ThrottleController switched to throttle state', undefined, 4.0));
        }
        else if (state === 'abort') {
            track(finding('CIRCUIT_BREAKER_ABORTED', 'GET', '/', 'Circuit breaker aborted the test', 'critical', 'ThrottleController switched to abort state — partial report', undefined, 8.0));
        }
        return findings;
    }
}
