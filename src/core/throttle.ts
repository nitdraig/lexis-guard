...
function createWindow(size: number): LatencyWindow {
  return { samples: [], maxSize: size };
}

function pushSample(win: LatencyWindow, value: number): void {
...
function calculateP95(win: LatencyWindow): number {
  if (win.samples.length === 0) return 0;
  const sorted = [...win.samples].sort((a, b) => a - b);
  return percentile(sorted, 95);
}

export class ThrottleController {
  private state: ThrottleState = 'normal';
  private currentConcurrency: number;
  private readonly baselineConcurrency: number;
  private readonly limits: ThrottleLimits;
  private readonly window: LatencyWindow;
  private baselineLatency = 0;
  private throttleCount = 0;
  /** Minimum samples in throttle before abort can trigger. */
  private readonly minThrottleSamples = 10;
