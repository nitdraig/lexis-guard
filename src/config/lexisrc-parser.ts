 ...
 */
function resolveAuthProfiles(raw: RawLexisrc['auth']['profiles']): Lexisrc['auth']['profiles'] {
  const resolved: Lexisrc['auth']['profiles'] = {};
 ...
 */
function validateMultiAuth(config: Lexisrc): string[] {
  const errors: string[] = [];
  const profiles = Object.entries(config.auth.profiles);

  if (profiles.length < 3) {
    errors.push(
      `Multi-auth requires at least 3 profiles (user_a, user_b, admin). Found ${profiles.length}.`
    );
    return errors;
  }
 ...
 */
export function parseLexisrc(content: string | unknown): ParseLexisrcResult {
  let raw: unknown;
  ...
  if (!strict.success) {
    const errors = strict.error.errors.map(
      (e) => `${e.path.join('.')}: ${e.message}`
    );
    return { ok: false, errors };
  }
 ...
 */
export function parseLexisrcStrict(content: string | unknown): Lexisrc {
